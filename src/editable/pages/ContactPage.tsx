      ...
      <main className="bg-[#FFF6DE] text-[#2f1d16]">
        <section className="border-b border-black bg-white">
          <div className="mx-auto max-w-[var(--editable-container)] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
            <p className="text-xs font-black uppercase tracking-[0.28em] text-[#F48F68]">{pagesContent.contact.eyebrow}</p>
            <h1 className="editorial-brand mt-4 max-w-3xl text-4xl font-black leading-[1.02] sm:text-6xl">{pagesContent.contact.title}</h1>
            <p className="mt-6 max-w-2xl border-l-4 border-[#F48F68] pl-5 text-base font-semibold leading-8 text-black/65">{pagesContent.contact.description}</p>
          </div>
        </section>

        <section className="mx-auto grid max-w-[var(--editable-container)] border-x border-black bg-white lg:grid-cols-[0.72fr_1.28fr]">
          <aside className="border-b border-black bg-[#8BDFDD] text-[#2f1d16] lg:border-b-0 lg:border-r">
            {desks.map((desk, index) => (
              <div key={desk.title} className="border-b border-white/25 p-7 last:border-b-0 sm:p-9">
                <div className="flex items-center justify-between"><desk.icon className="h-5 w-5 text-[#F48F68]" /><span className="text-xs font-black text-[#2f1d16]/45">0{index + 1}</span></div>
                <h2 className="editorial-serif mt-6 text-3xl font-black">{desk.title}</h2>
                <p className="mt-3 text-sm leading-7 text-[#2f1d16]/70">{desk.body}</p>
              </div>
            ))}
          </aside>
          <div className="p-6 sm:p-10 lg:p-14">
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[#F48F68]">Send a message</p>
