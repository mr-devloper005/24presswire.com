    ...
    <Link href={href} className={`group block overflow-hidden border-b border-r border-black bg-white ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
        <div className={`relative overflow-hidden bg-black ${strong ? 'aspect-[16/7]' : 'aspect-[16/10]'}`}>
          ...
          <span className="absolute left-4 top-4 bg-[#F48F68] px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-[#2f1d16]">{taskLabel}</span>
        </div>
      ) : null}
      <div className="p-5 sm:p-6">
        {!image ? <span className="bg-[#F48F68] px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-[#2f1d16]">{taskLabel}</span> : null}
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[#FFF6DE] text-[#2f1d16]">
        <section className="mx-auto max-w-[var(--editable-container)] border-x border-black bg-[#FFF6DE]">
          <div className="grid border-b border-black bg-white md:grid-cols-[0.8fr_1.2fr]">
            <div>
              <div className="h-full bg-[#F48F68] p-7 text-[#2f1d16] sm:p-10 lg:p-14">
                <p className="text-xs font-black uppercase tracking-[0.28em]">{pagesContent.search.hero.badge}</p>
                <h1 className="editorial-brand mt-5 text-4xl font-black leading-[1.02] sm:text-6xl">{pagesContent.search.hero.title}</h1>
                <p className="mt-6 max-w-xl text-base font-semibold leading-8 text-[#2f1d16]/75">{pagesContent.search.hero.description}</p>
              </div>
            </div>
            <form action="/search" className="self-center p-6 sm:p-10 lg:p-14">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center border border-black bg-[#2f1d16] px-6 text-xs font-black uppercase tracking-[0.18em] text-[#FFF6DE] transition hover:bg-[#F48F68] hover:text-[#2f1d16]" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="flex flex-wrap items-end justify-between gap-4 border-b-4 border-black px-4 py-8 sm:px-6 lg:px-8">
            <div>
              ...
              <h2 className="editorial-brand mt-2 text-3xl font-black">{query ? `Results for "${query}"` : pagesContent.search.resultsTitle}</h2>
            </div>
